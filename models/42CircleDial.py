...
def get_eliminate(thresh_image, threshold_max, threshold_min, reverse):
    ...
    if reverse:
        ...
        return eliminate_image
def get_42CircleDialvalue(image):
    ...
    if circles is not None:
        ...
        try:
            ...
            thresh_image = get_eliminate(thresh_image, 5000, 100, True)

            thresh_image = get_eliminate(thresh_image, 100, 80, False)
            thresh_image = get_length(thresh_image, 45, 20)
            #cv2.imshow("test", thresh_image)
            #cv2.waitKey(0)
            # 找尋所有線段
            ...
            left_bottom_select_lines =[linel for linel in merged_lines if
                                       thresh_image.shape[0] * 0.15 < min(linel[0][0],linel[0][2]) < thresh_image.shape[0] * 0.3 and
                                        thresh_image.shape[1] * 0.6 < min(linel[0][1], linel[0][3]) < thresh_image.shape[1] * 0.8]

            right_bottom_select_lines = [line for line in merged_lines if
                                         thresh_image.shape[0] * 0.65 < max(line[0][0],line[0][2]) < thresh_image.shape[0] * 0.85 and
            ...
            value = round(PointLineAngle/LineAngle ,3)
            print(value)
...
# 讀取影像
#image = cv2.imread('../Test/42CircleDial/CircleDialImage-1.png')

# 呼叫 get_value 函式
#result = get_42CircleDialvalue(image)
