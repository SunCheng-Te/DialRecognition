...
def find_point_on_line(merged_long_lines, extend_pointer_line, pointer_line):
    ...
    if len(nearest_point) == 0:
        nearest_point = int(image_height * 0.475),int(image_height * 0.525)#nearest_point = int(image_height * 0.5),int(image_height * 0.5)
...
def get_length(thresh_image, arc_length_max, arc_length_min):
    ...
    for contour in contours:
        ...
        if top_distance > max_r and bottom_distance > max_r and left_distance > max_r and right_distance > max_r:
            ...
            cv2.drawContours(eliminate_image, [contour], -1, 0, thickness=cv2.FILLED)
        elif top_point[1] < eliminate_image.shape[0] * 0.15 or top_point[1] > eliminate_image.shape[0] * 0.8:
            cv2.drawContours(eliminate_image, [contour], -1, 0, thickness=cv2.FILLED)
        elif top_point[0] < eliminate_image.shape[1] * 0.2 or top_point[0] > eliminate_image.shape[1] * 0.8:
            cv2.drawContours(eliminate_image, [contour], -1, 0, thickness=cv2.FILLED)
...
def get_eliminate(thresh_image, threshold_max, threshold_min, reverse):
    ...
    if reverse:
        ...
        return eliminate_image
def get_43LineCircleDialvalue(image):
    ...
    if circles is not None:
        ...
        try:
            ...
            thresh_image = cv2.adaptiveThreshold(
                gray_img, 255, cv2.ADAPTIVE_THRESH_MEAN_C, cv2.THRESH_BINARY, 17, 5
            )
            ...
            thresh_image = get_eliminate(thresh_image, 5000, 10, True)

            #thresh_image = get_eliminate(thresh_image, 30, 10, False)
            thresh_image = get_length(thresh_image, 45, 10)
            # cv2.namedWindow("test", cv2.WINDOW_NORMAL)
            # cv2.imshow("test", thresh_image)
            # cv2.waitKey(0)
            # 找尋所有線段
            lines = cv2.HoughLinesP(thresh_image, 1, np.pi / 180, 35, 30)
            ...
            left_bottom_select_lines =[linel for linel in merged_lines if
                                       thresh_image.shape[0] * 0.15 < min(linel[0][0],linel[0][2]) < thresh_image.shape[0] * 0.32 and
                                        thresh_image.shape[1] * 0.625 < min(linel[0][1], linel[0][3]) < thresh_image.shape[1] * 0.75]

            right_bottom_select_lines = [line for line in merged_lines if
                                         thresh_image.shape[0] * 0.6 < max(line[0][0],line[0][2]) < thresh_image.shape[0] * 0.7 and
                                         thresh_image.shape[1] * 0.6 < max(line[0][1],line[0][3]) < thresh_image.shape[1] * 0.75]
            ...
            LineAngle = round(calculate_angle(left_line, right_line))#*1.02
            value = round((PointLineAngle/LineAngle)*150,2)
            print(value)
            ...
            #         cv2.line(image, (x1, y1), (x2, y2), (0, 0, 255), 2)
            #     count =count+1
            # cv2.putText(image, f'Result: {str(value)}', (30, 50), cv2.FONT_HERSHEY_SIMPLEX, 1,
            #              (0, 0, 255), 1, cv2.LINE_AA)
            # image_path = 'Test\\43Line\\1709627164951.png'  # 輸入你的圖片路徑
...
#image_path = '../Test/43Line_all/43Line_11.png'
# 讀取影像
#image = cv2.imread(image_path)

# 呼叫 get_value 函式
#result = get_43LineCircleDialvalue(image)
