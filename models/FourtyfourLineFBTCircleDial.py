...
def find_point_on_line(merged_long_lines, extend_pointer_line, pointer_line):
    ...
    if len(nearest_point) == 0:
        ...
        #nearest_point = int(image_height * 0.5),int(image_height * 0.475)#
...
def get_length(thresh_image, arc_length_max, arc_length_min):
    ...
    for contour in contours:
        ...
        if top_distance > max_r and bottom_distance > max_r and left_distance > max_r and right_distance > max_r:
            ...
            cv2.drawContours(eliminate_image, [contour], -1, 0, thickness=cv2.FILLED)
        elif top_point[0] < eliminate_image.shape[1] * 0.15 or top_point[0] > eliminate_image.shape[1] * 0.8:
            cv2.drawContours(eliminate_image, [contour], -1, 0, thickness=cv2.FILLED)
...
def get_eliminate(thresh_image, threshold_max, threshold_min, reverse):
    ...
    if reverse:
        ...
        return eliminate_image
def get_44LineFBTCircleDialvalue(image):
        ...
        try:
            ...
            left_bottom_select_lines =[linel for linel in merged_lines if
                                       thresh_image.shape[0] * 0.15 < min(linel[0][0],linel[0][2]) < thresh_image.shape[0] * 0.32 and
                                        thresh_image.shape[1] * 0.6 < min(linel[0][1], linel[0][3]) < thresh_image.shape[1] * 0.85]

            right_bottom_select_lines = [line for line in merged_lines if
                                         thresh_image.shape[0] * 0.7 < max(line[0][0],line[0][2]) < thresh_image.shape[0] * 0.85 and
                                         thresh_image.shape[1] * 0.6 < max(line[0][1],line[0][3]) < thresh_image.shape[1] * 0.85]
            ...
            value = round((PointLineAngle/LineAngle)*1.0,3)
            print(value)
            ...
            # image_path = 'Test\\44LineFBT\\1709629041729.png'  # 輸入你的圖片路徑
...
image_path = '../Test/44LineFBT_all/44LineCircleGuageFBT_9.png'
# 讀取影像
image = cv2.imread(image_path)

# 呼叫 get_value 函式
result = get_44LineFBTCircleDialvalue(image)
